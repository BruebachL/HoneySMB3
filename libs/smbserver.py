...
# For signing
from . import smb, nmb, ntlm, uuid, LOG
from . import smb3structs as smb2
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            ...
            if createDisposition == smb.FILE_SUPERSEDE:
                ...
                if os.path.exists(pathName) is not True and (
                        six.text_type(pathName) in smbServer.getRegisteredNamedPipes()) is not True:
                    errorCode = STATUS_NO_SUCH_FILE
                ...
                if errorCode == STATUS_SUCCESS:
                    try:
                        if os.path.isdir(pathName) and sys.platform == 'win32':
                            ...
                            if sys.platform == 'win32':
                                mode |= os.O_BINARY
                            if six.text_type(pathName) in smbServer.getRegisteredNamedPipes():
                                fid = PIPE_FILE_DESCRIPTOR
                                sock = socket.socket()
                                sock.connect(smbServer.getRegisteredNamedPipes()[six.text_type(pathName)])
                            else:
                                fid = os.open(pathName, mode)
                    except Exception as e:
                        smbServer.log("NTCreateAndX: %s,%s,%s" % (pathName, mode, e), logging.ERROR)
...
class SMBSERVERHandler(six.moves.socketserver.BaseRequestHandler):
    ...
    def finish(self):
        ...
        return six.moves.socketserver.BaseRequestHandler.finish(self)


class SMBSERVER(six.moves.socketserver.ThreadingMixIn, six.moves.socketserver.TCPServer):
    ...
    def registerNamedPipe(self, pipeName, address):
        self.__registeredNamedPipes[six.text_type(pipeName)] = address
        return True

    def unregisterNamedPipe(self, pipeName):
        if pipeName in self.__registeredNamedPipes:
            del (self.__registeredNamedPipes[six.text_type(pipeName)])
            return True
        return False
                        ...
                        if self.__SMB2Support is True:
                            if packet['Command'] == smb.SMB.SMB_COM_NEGOTIATE:
                                try:
                                    ...
                                    # If something went wrong, let us fallback to SMB1
                                    respCommands, respPackets, errorCode = self.__smbCommands[packet['Command']](
                                        connId,
                                        self,
                                        SMBCommand,
    ...
    def processConfigFile(self, configFile=None):
        ...
        if self.__serverConfig is None:
            if configFile is None:
                configFile = 'smb.conf'
            self.__serverConfig = six.moves.configparser.ConfigParser()
